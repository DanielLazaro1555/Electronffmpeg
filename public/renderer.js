...
ipcRenderer.on(
  "url-analizada",
  (event, { fileType, resolution, format, codec, fps, bitrate }) => {
    document.getElementById("file-info").style.display = "block"; // Mostrar la información

    document.getElementById(
      "file-type"
    ).innerText = `Tipo de archivo: ${fileType}`;
    document.getElementById("file-resolution").innerText = resolution
    ...
    document.getElementById("file-fps").innerText = `FPS: ${
      fps || "No disponible"
    }`;
    document.getElementById("file-bitrate").innerText = `Tasa de bits: ${
...
ipcRenderer.on("progreso-descarga", (event, data) => {
  ...
  if (typeof data === "string" && data.startsWith("Archivo descargando")) {
    ...
    document.getElementById("file-type").innerText = `Tipo de archivo: ${
      data.fileType || "Desconocido"
    }`;
    document.getElementById("file-resolution").innerText = `Resolución: ${
      data.resolution || "No disponible"
    }`;
    document.getElementById("file-format").innerText = `Formato: ${
      data.format || "No disponible"
    }`;
    document.getElementById("file-codec").innerText = `Códec: ${
      data.codec || "No disponible"
    }`;
    document.getElementById("file-fps").innerText = `FPS: ${
      data.fps || "No disponible"
    }`;
    document.getElementById("file-bitrate").innerText = `Tasa de bits: ${
      data.bitrate || "No disponible"
    }`;
  } else {
